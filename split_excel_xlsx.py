...
for file_index in range(num_files):
    ...
    sheetq = xlsx2.active  # 获取默认的工作表

    # 复制表头
    for col in range(total_cols):
        sheetq.cell(row=1, column=col + 1, value=data[0][col])

    # 复制数据
    for row_offset, row_index in enumerate(range(start_row, end_row)):
        for col in range(total_cols):
            sheetq.cell(row=row_offset + 2, column=col + 1, value=data[row_index][col])

    # 生成文件名
    file_name = os.path.join(save_add, f"拆分表_{file_index + 1}_{time.strftime('%Y-%m-%d')}.xlsx")
